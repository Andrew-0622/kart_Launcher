                ...
                while (!pinRestored && checkCount < 30) // 最多检测30秒
                {
                    if (CheckTcpConnection(processId, serverIP, serverPort))
                    {
                        ...
                        if (RestorePinFile())
                        {
                            ...
                            // 修改内存
                            // 移除星标赛道数量限制
                            ModifyMemory(processId, new byte[] { 0x83, 0xFA, 0x32, 0x7C, 0x0D, 0x8B, 0x45, 0x10, 0xC6, 0x00, 0x02, 0x32, 0xC0, 0xE9, 0xCC, 0x00, 0x00, 0x00 },
                            new byte[] { 0x83, 0xFA, 0x32, 0x7C, 0x0D, 0x8B, 0x45, 0x10, 0xC6, 0x00, 0x02, 0x32, 0xC0, 0x90, 0x90, 0x90, 0x90, 0x90 });
                            // 赛道模型边界大小2000改为10000单浮点
                            ModifyMemory(processId, new byte[] { 0x00, 0x00, 0x98, 0x41, 0x00, 0x00, 0xFA, 0x44 },
                            new byte[] { 0x00, 0x00, 0x98, 0x41, 0x00, 0x40, 0x9C, 0x46 });
